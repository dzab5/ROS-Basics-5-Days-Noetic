...
rospy.init_node('crash_service_client') # initialise a ROS node with the name service_client
service_name = "/crash_server"
...
while not ctrl_c:
    ...
    if result.success:
        ...
        rospy.logwarn("In Direction=="+str(result.message)) # print the result given by the service called
    else:
        rospy.loginfo("Success =="+str(result.success)) # print the result given by the service called
        rospy.loginfo("In Direction=="+str(result.message)) # print the result given by the service called
    rate.sleep()